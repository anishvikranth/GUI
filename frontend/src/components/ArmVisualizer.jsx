...
export function ArmVisualizer() {
  ...
  const connected = roverState.connected;
  const isAutonomous = roverState.arm_state;   // Maps to Pydantic: arm_state
  const isPosition = roverState.input_space; // true → position space, false → joint space

  // hook returns all angles in DEGREES
  const { base, shoulder, elbow, pitch, roll, gripper } = roverState.encoder_angles ?? {
    base: 0,
